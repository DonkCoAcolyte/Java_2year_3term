...
// как компонент, совместно используемый в различных методах
    ...
    public Double calculate1(Double x, Double y, Double z) {
        return Math.pow(Math.cos(Math.exp(x)) + Math.log((1+y)*(1+y)) + Math.sqrt(Math.exp(Math.cos(x)) + Math.sin(3.14 * z) * Math.sin(3.14 * z)) + Math.sqrt(1/x) + Math.cos(y*y), Math.sin(z));
    }
...
// Создать область для вывода результата
        ...
        //hboxResult.add(Box.createHorizontalStrut(60));
...
// Создать область для кнопок
        ...
        JButton buttonMPlus = new JButton ("M+"); // make it remember sum
